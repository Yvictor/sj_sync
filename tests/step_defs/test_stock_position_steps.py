...
@given(parsers.parse("我有昨日資買庫存 {code} 數量 {quantity:d} 張"), target_fixture="initial_position")
...
@given(parsers.parse("我有昨日券賣庫存 {code} 數量 {quantity:d} 張"), target_fixture="initial_position")
...
@then(parsers.parse("資買庫存應為 {expected_qty:d} 張，昨日庫存為 {expected_yd:d} 張，昨日已抵銷 {expected_offset:d} 張"))
def check_margin_position_with_yd_offset(position_sync, context, expected_qty, expected_yd, expected_offset):
    ...
    assert position.quantity == expected_qty, f"資買庫存應為 {expected_qty} 張，實際為 {position.quantity} 張"
    assert position.yd_quantity == expected_yd, f"昨日庫存應為 {expected_yd} 張，實際為 {position.yd_quantity} 張"
    assert position.yd_offset_quantity == expected_offset, f"昨日已抵銷應為 {expected_offset} 張，實際為 {position.yd_offset_quantity} 張"
...
@then(parsers.parse("券賣庫存應為 {expected_qty:d} 張，昨日庫存為 {expected_yd:d} 張，昨日已抵銷 {expected_offset:d} 張"))
def check_short_position_with_yd_offset(position_sync, context, expected_qty, expected_yd, expected_offset):
    ...
    assert position.quantity == expected_qty, f"券賣庫存應為 {expected_qty} 張，實際為 {position.quantity} 張"
    assert position.yd_quantity == expected_yd, f"昨日庫存應為 {expected_yd} 張，實際為 {position.yd_quantity} 張"
    assert position.yd_offset_quantity == expected_offset, f"昨日已抵銷應為 {expected_offset} 張，實際為 {position.yd_offset_quantity} 張"
...
def check_cash_position_with_yd(position_sync, context, expected_qty, expected_yd):
    ...
    assert position.quantity == expected_qty, f"現股庫存應為 {expected_qty} 張，實際為 {position.quantity} 張"
    assert position.yd_quantity == expected_yd, f"昨日庫存應為 {expected_yd} 張，實際為 {position.yd_quantity} 張"
...
def check_cash_sell_position(position_sync, context, expected_qty):
    ...
    if expected_qty == 0:
        ...
        assert position.quantity == expected_qty, f"現股賣出庫存應為 {expected_qty} 張，實際為 {position.quantity} 張"
