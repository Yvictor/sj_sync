...
功能: 混合交易場景
  ...
  場景: 有昨日資買 - 當天券賣超過當天資買數量
    ...
    而且 券賣庫存應為 5 張
    # 券賣先抵銷當天資買 3 張（當沖）
    # 剩餘 5 張券賣**不能**抵銷昨日資買（台灣規則：非當沖時券賣只能用券買平倉）
    # 所以剩餘 5 張券賣會建立新的券賣庫存
    # 資買庫存：quantity=10（昨日的10張保持不變）, yd_quantity=10, yd_offset_quantity=0
    # 券賣庫存：quantity=5（新建立的券賣部位）

  場景: 有昨日券賣 - 當天資買超過當天券賣數量
    假設 我有昨日券賣庫存 2330 數量 10 張
    當 我券賣 2330 數量 3 張
    那麼 券賣庫存應為 13 張，昨日庫存為 10 張，昨日已抵銷 0 張
    當 我資買 2330 數量 8 張
    那麼 券賣庫存應為 10 張，昨日庫存為 10 張，昨日已抵銷 0 張
    而且 資買庫存應為 5 張
    # 資買先抵銷當天券賣 3 張（當沖）
    # 剩餘 5 張資買**不能**抵銷昨日券賣（台灣規則：非當沖時資買只能用資賣平倉）
    # 所以剩餘 5 張資買會建立新的資買庫存
    # 券賣庫存：quantity=10（昨日的10張保持不變）, yd_quantity=10, yd_offset_quantity=0
    # 資買庫存：quantity=5（新建立的資買部位）
