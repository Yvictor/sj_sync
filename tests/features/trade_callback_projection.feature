...
# UpdatePrice 成功             status.modified_price             order.price；其他交易狀態不變
# UpdateQty 成功               status.cancel_quantity            status.cancel_quantity；order.quantity 保留原始委託量
# Cancel 成功                  status.cancel_quantity            status.status = Cancelled、status.cancel_quantity
...
#
# 六個必要觀察欄位：
# status.status、order.price、order.quantity、status.cancel_quantity、
# status.deal_quantity、status.deals
...
功能: 主動回報投影到 Native Trade 欄位
  ...
  場景: New 成功只把新單狀態更新為 Submitted
    ...
    那麼 Trade 六個欄位應為 status Submitted、order.price 100.0、order.quantity 2、cancel_quantity 0、deal_quantity 0、deals 0 筆
  ...
  場景: New 失敗把狀態更新為 Failed 並保留委託欄位
    ...
    那麼 Trade 六個欄位應為 status Failed、order.price 100.0、order.quantity 2、cancel_quantity 0、deal_quantity 0、deals 0 筆
    而且 Trade status_code 應為 E001，msg 應為 rejected

  場景: UpdatePrice 成功使用 modified_price 更新 order.price
    假設 已收到 New 成功回報
    當 收到 UpdatePrice 委託回報，op_code 00，order.price 100.0，order.quantity 2，modified_price 101.0，cancel_quantity 0
    那麼 Trade 六個欄位應為 status Submitted、order.price 101.0、order.quantity 2、cancel_quantity 0、deal_quantity 0、deals 0 筆
    而且 使用者 callback 觀察到 status Submitted、order.price 101.0、order.quantity 2、cancel_quantity 0、deal_quantity 0、deals 0 筆
  ...
  場景: UpdateQty 成功保留原始委託量並增加 cancel_quantity
    ...
    那麼 Trade 六個欄位應為 status Submitted、order.price 100.0、order.quantity 2、cancel_quantity 1、deal_quantity 0、deals 0 筆

  場景: 第一筆 Deal 把 Trade 更新為 PartFilled 並新增 Deal
    假設 已收到 New 成功回報
    當 收到成交回報 exchange_seq D001，price 100.5，quantity 1
    那麼 Trade 六個欄位應為 status PartFilled、order.price 100.0、order.quantity 2、cancel_quantity 0、deal_quantity 1、deals 1 筆
  ...
  場景: 累計成交量達原始委託量時更新為 Filled
    ...
    那麼 Trade 六個欄位應為 status Filled、order.price 100.0、order.quantity 2、cancel_quantity 0、deal_quantity 2、deals 2 筆
  ...
  場景: Cancel 成功保留已成交資料並取消剩餘數量
    ...
    那麼 Trade 六個欄位應為 status Cancelled、order.price 100.0、order.quantity 2、cancel_quantity 1、deal_quantity 1、deals 1 筆
